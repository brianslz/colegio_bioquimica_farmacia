              ...
              <div class="col-md-10">
                <div class="input-group">
                  ...
                  <input
                    type="text"
                    v-model="buscar"
                    @keyup.enter="listarAfiliado(1,buscar,criterio)"
                    class="form-control"
                    placeholder="Texto a buscar"
                  >
                  <button type="submit" @click="listarAfiliado(1,'','ci')" class="btn btn-default">
                    <i class="fa fa-close"></i>
                  </button>
                  <button
                    type="submit"
                    @click="listarAfiliado(1,buscar,criterio)"
                ...
                <tbody>
                  <tr v-for="afiliado in arrayAfiliado" :key="afiliado.id">
                    ...
                    <td>
                      <button
                        type="button"
                        @click="mostrarListado(afiliado)"
                        v-tooltip.bottom="'Ver Aportes'"
                        class="btn btn-primary btn-sm"
                      >
                        <i class="icon-eye"></i>
                      </button>
                      <button
                        v-if="afiliado.condicion"
                        type="button"
                        @click="abrirModal('afiliado','registrar',afiliado)"
                        v-tooltip.bottom="'Registrar Aporte'"
          ...
          <div class="card-body">
            <div class="row">
              ...
              <div class="col-md-4">
                ...
                <div class="border">
                  ...
                  {{ desde(fecha_ultimo_pago) }}
                  <br>
                  <br>

                  <button
                    v-if="actualDeuda(fecha_ultimo_pago)"
                    type="button"
                  ...
                  <button
                    v-if="actualDeuda(fecha_ultimo_pago) && actualPromo(fecha_ultimo_pago)"
                    @click="pagarGestion()"
                    v-tooltip.bottom="'Valido Hasta el 31 de Marzo'"
                    type="button"
                    class="btn btn-primary btn-sm"
                  >
                  ...
                  <template v-if="calcularDeuda(fecha_ultimo_pago)>2 ">
                    <strong>Meses a Deber:</strong>
                    {{calcularDeuda(fecha_ultimo_pago)}} Meses
                    <br>
                    <strong>Costo a Pagar:</strong>
                    {{calcularDeuda(fecha_ultimo_pago)*20}} Bs
                  </template>
                </div>
              </div>
            </div>
            ...
            <div v-for="(aporte, index) in arrayAportes" :key="aporte.id">
              <table class="table table-bordered table-sm">
                <tr>
                  <th>Modaliad:</th>
                  <td>{{ aporte.modalidad }}</td>
                  <th>Comprobante:</th>
                  <td>{{aporte.codigo_verficacion}}</td>
                  <th>Fecha de Pago:</th>
                  <td>{{ desde(aporte.fecha_pago) }}</td>
                </tr>
                <tr>
                  <th>Numero de Meses:</th>
                  <td>{{aporte.num_meses}}</td>
                  <th>Monto:</th>
                  <td>{{aporte.monto}}</td>
                  <th>Vale hasta:</th>
            ...
            <div class="form-group row">
              <div class="col-md-6">
                ...
                <button type="button" @click="reporteAportes()" class="btn btn-success">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action method="post" enctype="multipart/form-data" class="form-horizontal">
              ...
              <div class="form-group row" v-if="promo">
                <!--formulario promocion de un año-->
                <label class="col-md-3 form-control-label" for="text-input">Numero de Meses (*)</label>
                <div class="col-md-3">
                  <input type="number" v-model="num_meses" class="form-control">
                </div>
              ...
              <div v-if="num_meses > 0" class="form-group row">
                <label
                  ...
                  for="text-input"
                >Duracion del pago sera hasta :</label>
                <div class="col-md-3">
              ...
              <div v-show="errorAfiliado" class="form-group row div-error">
                <div class="text-center text-error">
                  <div v-for="error in errorMostrarMsjAfiliado" :key="error" v-text="error"></div>
                </div>
              </div>
            </form>
          </div>
...
export default {
  ...
  methods: {
    ...
    validarAfiliado() {
      ...
      if (!this.fecha_pago)
        this.errorMostrarMsjAfiliado.push("Introduce la Fecha");
      if (!this.codigo_verficacion)
        this.errorMostrarMsjAfiliado.push("Introduce el numero de comprobante");
      if (!this.num_meses)
        this.errorMostrarMsjAfiliado.push(
          "Introduce el numero de meses a pagar"
        );
    ...
    cargarPdf() {
      window.open(
        "http://127.0.0.1:8000/pago/reciboPdf?id=" + this.idafiliado,
        "_blank"
      );
    },
    reporteAportes() {
      window.open(
        "http://127.0.0.1:8000/pago/aportesPdf?id=" + this.idafiliado,
        "_blank"
      );
    },
